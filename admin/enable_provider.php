...
// Reactivar proveedor
$stmt = $pdo->prepare("UPDATE providers SET active = 1 WHERE id = ? AND company_id = ?");
$stmt->execute([$id, $company_id]);

// Redirigir con mensaje de éxito y mantener filtro de inactivos
header("Location: provider.php?filtro=inactivos&enabled=1");
exit;
